...
//     localStorage.setItem(`notificationBannerDismissed_${userType}_${userId}`, Date.now().toString());
//   };
...
//   ? localStorage.getItem(`notificationBannerDismissed_${userType}_${userId}`)
//   : null;
...
export default function NotificationPermission({
  ...
  const bannerKey = `notificationBannerDismissed_${userType}_${userId}`;
  const ownerKey = 'activeNotificationOwner';

  useEffect(() => {
    if (!isNotificationSupported() || !userId) return;

    const currentPermission = getNotificationPermission();
    setPermission(currentPermission);

    const dismissed =
      typeof window !== 'undefined' ? localStorage.getItem(bannerKey) : null;
    ...
    if (currentPermission === 'granted' && userType === 'vendor') {
      registerToken();
    }
  }, [userType, userId]);
  ...
  const registerToken = async () => {
    try {
      ...
      const existingOwnerRaw =
        typeof window !== 'undefined' ? localStorage.getItem(ownerKey) : null;

      const existingOwner = existingOwnerRaw ? JSON.parse(existingOwnerRaw) : null;

      if (
        existingOwner &&
        existingOwner.userType === userType &&
        existingOwner.userId === userId &&
        existingOwner.token === fcmToken
      ) {
        console.log('ℹ️ Token already registered for this owner');
        onTokenReceived?.(fcmToken);
        return;
      }
      ...
      localStorage.setItem(
        ownerKey,
        JSON.stringify({ userType, userId, token: fcmToken })
      );
    ...
    try {
      ...
      if (fcmToken) {
        ...
        if (userType === 'vendor') {
          ...
          if (!data.success) {
            console.error('❌ Failed to register token:', data.error);
            alert(data.error || 'Failed to enable notifications');
            return;
          }
        }
