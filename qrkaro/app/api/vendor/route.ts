...
export async function POST(req: NextRequest) {
  ...
  if (!menuItems || !Array.isArray(menuItems) || menuItems.length === 0) {
    ...
    // });
    // ── Generate QR Code ──────────────────────────────────────────────────────
const customerUrl = shopType === 'Xerox Shop'? `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/x/${vendorId}`: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/v/${vendorId}`;
...
const qrCodeDataUrl = await QRCode.toDataURL(customerUrl, {
  ...
  color: {
    ...
    const vendorData = {
      ...
      state,
      shopType,
      menuItems: shopType === 'Xerox Shop' ? [] : menuItems,
      // menuItems,
      upiId,
      accountHolderName,
