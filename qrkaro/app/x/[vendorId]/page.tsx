...
export default function XeroxCustomerPage() {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<
    { fileUrl: string; fileName: string; pageCount: number }[]
  >([]);
  const [uploadError, setUploadError] = useState('');
  const [dragOver, setDragOver] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileChange = async (files: FileList | File[]) => {
    ...
    for (const f of fileArray) {
      if (f.type !== 'application/pdf') { setUploadError(`${f.name}: Only PDF files are allowed.`); continue; }
      if (f.size > 25 * 1024 * 1024) { setUploadError(`${f.name}: File too large. Max 25MB.`); continue; }
      setUploadError('');
      setUploading(true);
  ...
  const calcFileTotal = (
    ...
    if (!xeroxSettings) return 0;
    const total = filePageCount;
    let pages = total;
    if (fileps.pageRange !== 'all' && fileps.pageRange.trim()) {
      try {
        let count = 0;
        for (const part of fileps.pageRange.split(',').map(s => s.trim())) {
          if (part.includes('-')) {
            const [s, e] = part.split('-').map(Number);
            if (!isNaN(s) && !isNaN(e) && s >= 1 && e <= total) count += e - s + 1;
          } else {
            const p = Number(part);
            if (!isNaN(p) && p >= 1 && p <= total) count++;
          }
        }
        pages = count > 0 ? count : total;
      } catch { pages = total; }
    }
    const basePerPage = fileps.colorMode === 'color'
      ? xeroxSettings.colorPerPage
      : fileps.colorMode === 'grayscale'
        ? (xeroxSettings.grayscalePerPage ?? 3)
        : xeroxSettings.bwPerPage;
    const qualityExtra = fileps.colorMode === 'color' && fileps.printQuality === 'high' ? 2 : 0;
    const sizeMulti = PAPER_MULTIPLIER[fileps.paperSize] ?? 1;
    const dsDiscount = fileps.doubleSided ? 0.85 : 1;
    let total2 = pages * (basePerPage + qualityExtra) * sizeMulti * dsDiscount * fileps.copies;
    if (fileps.stapling) total2 += 5;
    if (fileps.binding) total2 += 20;
    return Math.ceil(total2);
  };

  const totalAmount = uploadedFiles.reduce((sum, f) => sum + calcFileTotal(f.pageCount, ps), 0);
  const effectivePages = uploadedFiles.reduce((sum, f) => sum + f.pageCount, 0);
  const canSubmit = uploadedFiles.length > 0 && !!customerName.trim() && customerPhone.trim().length === 10 && totalAmount > 0;
    ...
    try {
      const orderRes = await fetch('/api/xerox/payment/create', {
        ...
        body: JSON.stringify({
          amount: totalAmount,
          vendorId,
          fileName: `${uploadedFiles.length} file(s)`,
        }),
      });
      const orderData = await orderRes.json();
      if (!orderRes.ok) throw new Error(orderData.error);
      ...
      const options = {
        ...
        currency: 'INR',
        name: shopName,
        description: `Print: ${uploadedFiles.length} file(s)`,
        order_id: orderData.orderId,
        prefill: { name: customerName, contact: customerPhone },
        theme: { color: '#FF5A00' },
        handler: async (response: any) => {
          const verifyRes = await fetch('/api/xerox/payment/verify', {
            ...
            body: JSON.stringify({
              ...
              customerName: customerName || 'Walk-in Customer',
              customerPhone,
              // ── primary file (legacy compat) ──
              fileUrl: uploadedFiles[0].fileUrl,
              fileName: uploadedFiles.map(f => f.fileName).join(', '),
              pageCount: uploadedFiles.reduce((s, f) => s + f.pageCount, 0),
              // ── all files array ──
              ...
              // ── print settings (apply to all files) ──
              printType: ps.colorMode === 'color' ? 'color' : 'bw',
              copies: ps.copies,
              doubleSided: ps.doubleSided,
              colorMode: ps.colorMode,
              paperSize: ps.paperSize,
              orientation: ps.orientation,
              pageRange: ps.pageRange === 'all' ? '' : ps.pageRange,
              printQuality: ps.printQuality,
              stapling: ps.stapling,
              binding: ps.binding,
              specialInstructions: ps.specialInstructions,
      ...
      <div className="max-w-lg mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            ...
            {uploadedFiles.map((f, i) => (
              <div key={i} className="flex items-center gap-3 bg-green-50 border border-green-100 rounded-2xl p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-green-600 font-semibold">{f.pageCount} pages detected ✓</p>
                </div>
                <button
            ...
            <div
              onDrop={e => { e.preventDefault(); setDragOver(false); if (e.dataTransfer.files.length) handleFileChange(e.dataTransfer.files); }}
              onDragOver={e => { e.preventDefault(); setDragOver(true); }}
              onDragLeave={() => setDragOver(false)}
              onClick={() => !uploading && fileRef.current?.click()}
              className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-all ${dragOver ? 'border-[#FF5A00] bg-orange-50' : 'border-gray-200 hover:border-[#FF5A00] hover:bg-orange-50/30'
                }`}
            >
              <div className="w-12 h-12 bg-orange-50 rounded-2xl flex items-center justify-center mx-auto mb-2">
                <Upload size={22} className="text-[#FF5A00]" />
              </div>
              <p className="text-sm font-bold text-gray-700 mb-0.5">
                {uploading ? 'Uploading...' : uploadedFiles.length > 0 ? '+ Add another PDF' : 'Tap to upload PDF'}
              </p>
              <p className="text-xs text-gray-400">PDF only · Max 25MB · Multiple files allowed</p>
              {uploading && (
                <div className="mt-3">
                  <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#FF5A00] rounded-full transition-all duration-500" style={{ width: `${uploadProgress}%` }} />
            ...
            {/* KEY CHANGE: multiple attribute added */}
            <input
              ref={fileRef}
              type="file"
              ...
              multiple
              className="hidden"
              onChange={e => { if (e.target.files?.length) handleFileChange(e.target.files); e.target.value = ''; }}
            />
          </div>
        </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {([
                    ...
                    <button key={m.value} onClick={() => set('colorMode', m.value)}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-2xl border-2 transition-all ${ps.colorMode === m.value ? 'border-[#FF5A00] bg-orange-50' : 'border-gray-100 hover:border-gray-200'
                        }`}>
                      <span className="text-xl">{m.emoji}</span>
                      <span className={`text-[10px] font-black text-center leading-tight ${ps.colorMode === m.value ? 'text-[#FF5A00]' : 'text-gray-600'}`}>
                        {m.label}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={q.value} onClick={() => set('printQuality', q.value)}
                      className={`flex-1 py-2.5 px-2 rounded-xl border-2 transition-all ${ps.printQuality === q.value ? 'border-[#FF5A00] bg-orange-50' : 'border-gray-100'
                        }`}>
                      <p className={`text-xs font-black ${ps.printQuality === q.value ? 'text-[#FF5A00]' : 'text-gray-700'}`}>{q.label}</p>
                      <p className="text-[10px] text-gray-400">{q.sub}</p>
                    </button>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {(['A4', 'A3', 'Legal', 'Letter'] as const).map((size) => (
                    <button key={size} onClick={() => set('paperSize', size)}
                      className={`py-2.5 rounded-xl border-2 text-xs font-black transition-all ${ps.paperSize === size ? 'border-[#FF5A00] bg-orange-50 text-[#FF5A00]' : 'border-gray-100 text-gray-600'
                        }`}>
                      {size}
                      {size === 'A3' && <span className="block text-[9px] opacity-60 font-medium">2× price</span>}
                    </button>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={o.value} onClick={() => set('orientation', o.value)}
                      className={`flex-1 flex items-center gap-2 py-2.5 px-4 rounded-xl border-2 transition-all ${ps.orientation === o.value ? 'border-[#FF5A00] bg-orange-50' : 'border-gray-100'
                        }`}>
                      <span>{o.icon}</span>
                      <span className={`text-xs font-bold ${ps.orientation === o.value ? 'text-[#FF5A00]' : 'text-gray-700'}`}>{o.label}</span>
                    </button>
          ...
          {open.copies && (
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-1.5 ml-2">
                    {[1, 2, 5, 10].map((n) => (
                      <button key={n} onClick={() => set('copies', n)}
                        className={`w-8 h-8 rounded-lg text-xs font-black transition-all ${ps.copies === n ? 'bg-[#FF5A00] text-white' : 'bg-gray-100 text-gray-500 hover:bg-orange-50'
                          }`}>{n}</button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2.5">
                  <button onClick={() => set('pageRange', 'all')}
                    className={`flex-1 py-2.5 rounded-xl border-2 text-xs font-black transition-all ${ps.pageRange === 'all' ? 'border-[#FF5A00] bg-orange-50 text-[#FF5A00]' : 'border-gray-100 text-gray-600'
                      }`}>
                    All Pages {effectivePages > 0 ? `(${effectivePages})` : ''}
                  </button>
                  <button onClick={() => set('pageRange', '')}
                    className={`flex-1 py-2.5 rounded-xl border-2 text-xs font-black transition-all ${ps.pageRange !== 'all' ? 'border-[#FF5A00] bg-orange-50 text-[#FF5A00]' : 'border-gray-100 text-gray-600'
                      }`}>
...
{uploadedFiles.length > 0 && (
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div className="p-4 space-y-2 text-sm">
      {[
        ['Files', `${uploadedFiles.length} file(s)`],
        ['Pages', ps.pageRange === 'all' ? `${effectivePages} pages` : `${effectivePages} selected`],
                ['Color Mode', ps.colorMode === 'bw' ? 'Black & White' : ps.colorMode === 'color' ? 'Full Color' : 'Grayscale'],
                ['Paper Size', ps.paperSize + (ps.paperSize === 'A3' ? ' (2×)' : '')],
                ['Orientation', ps.orientation.charAt(0).toUpperCase() + ps.orientation.slice(1)],
                ['Quality', ps.printQuality.charAt(0).toUpperCase() + ps.printQuality.slice(1)],
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-5 pt-3 bg-gradient-to-t from-[#F8F9FB] via-[#F8F9FB]/90 to-transparent">
        <div className="max-w-lg mx-auto">
          {uploadedFiles.length === 0 && <p className="text-center text-xs text-gray-400 mb-2 font-medium">Upload a PDF file to see pricing</p>}
          <button onClick={handlePay} disabled={!canSubmit || paying}
            className="w-full py-4 bg-[#FF5A00] disabled:bg-gray-200 disabled:text-gray-400 text-white font-black text-base rounded-2xl shadow-xl shadow-orange-200 active:scale-95 transition-all flex items-center justify-center gap-2">
            {paying ? (
              <>
